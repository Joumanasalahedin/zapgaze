...
def main():
    ...
    print(
        f"PostgreSQL target: {postgres_url.split('@')[1] if '@' in postgres_url else 'hidden'}"
    )
    ...
    try:
        ...
        migrate_table(sqlite_cursor, pg_cursor, "users", ["id", "name", "birthdate"])
        ...
        migrate_table(
            sqlite_cursor, pg_cursor, "results", ["id", "session_id", "data"], "id"
        )
