...
def calculate_gaze_features(samples):
    ...
    for i in range(1, len(gaze_points)):
        ...
        if dist > FIXATION_THRESHOLD:
            ...
            fixation_duration = timestamps[i-1] - \
                timestamps[current_fixation_start]
            ...
            current_fixation_center = (
                current_fixation_center + gaze_points[i]) / 2
    ...
    mean_fixation_duration = np.mean(
        fixation_durations) if fixation_durations else None
    ...
    for i in range(1, len(fixations)):
        ...
        distance = np.linalg.norm(
            curr_fixation['center'] - prev_fixation['center'])

        # Calculate time between fixations
        time_diff = timestamps[curr_fixation['start']] - \
            timestamps[prev_fixation['end']]
    ...
    return {
        'mean_fixation_duration': float(mean_fixation_duration) if mean_fixation_duration is not None else None,
...
def compute_session_features(session_uid: str, db: Session = Depends(get_db)):
    ...
    for i, event in enumerate(events):
        print(f"  {i}: {event.event_type}, stimulus={event.stimulus}, response={event.response}, timestamp={event.timestamp}")
    ...
    for stim in go_stimuli:
        ...
        matching_responses = [
            r for r in responses if r.timestamp >= stim.timestamp]
        if matching_responses:
            ...
            omission += 1
            print(
                f"DEBUG: Go trial '{stim.stimulus}' - No response found (omission)")
