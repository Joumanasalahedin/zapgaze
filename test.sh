...
curl -s -X POST $AGENT/start \
  -H "Content-Type: application/json" \
  -d "{\"session_uid\":\"$SESSION_UID\",\"api_url\":\"$API/acquisition/batch\",\"fps\":20}" \
  | jq
echo
...
curl -s -X POST $API/acquisition/batch \
  -H "Content-Type: application/json" \
  -d "[ 
    {\"session_uid\":\"$SESSION_UID\",\"timestamp\":1000,\"left_eye\":{\"x\":100,\"y\":100},\"right_eye\":{\"x\":105,\"y\":100},\"ear\":0.3,\"blink\":false},
    {\"session_uid\":\"$SESSION_UID\",\"timestamp\":1020,\"left_eye\":{\"x\":101,\"y\":101},\"right_eye\":{\"x\":106,\"y\":101},\"ear\":0.28,\"blink\":true}
  ]" \
  | jq
echo
...
curl -s -X POST $API/session/event \
  -H "Content-Type: application/json" \
  -d "{\"session_uid\":\"$SESSION_UID\",\"timestamp\":1000.0,\"event_type\":\"stimulus_onset\",\"stimulus\":\"O\"}"
echo
# Correct response
curl -s -X POST $API/session/event \
  -H "Content-Type: application/json" \
  -d "{\"session_uid\":\"$SESSION_UID\",\"timestamp\":1000.35,\"event_type\":\"response\",\"stimulus\":\"O\",\"response\":true}"
echo
# Stimulus onset (No-Go = X)
curl -s -X POST $API/session/event \
  -H "Content-Type: application/json" \
  -d "{\"session_uid\":\"$SESSION_UID\",\"timestamp\":1050.0,\"event_type\":\"stimulus_onset\",\"stimulus\":\"X\"}"
echo
